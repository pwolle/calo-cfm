...
class AttentionBlock(fj.Module, replace=True):
    __module_name = "_transformer3.AttentionBlock"
    ...
    cap: float = fj.field(static=True, default=20)
    ...
    def __call__(
        ...
        q = softcap(q, self.cap)
        k = softcap(k, self.cap)
        v = softcap(v, self.cap)
...
class MLPBlock(fj.Module, replace=True):
    __module_name = "_transformer3.MLPBlock"
    ...
    def init(cls, key: PRNGKeyArray, dim: int):
        ...
        return cls(
            ...
            w2=fn.Linear.init(key2, dim, dim),
    ...
    def __call__(
        ...
        x = jnn.gelu(x1, approximate=True) * x2
...
class Transformer(fj.Module, replace=True):
    __module_name = "_transformer3.Transformer"
